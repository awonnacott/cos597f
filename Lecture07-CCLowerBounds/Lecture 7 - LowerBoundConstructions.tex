...
\input{macros}
\begin{document}
\lecture{7}{Communication Lower Bounds}{Put Scribe Name Here}
\disclaimer 
\section{Logistics}
...
We'll also want a canonical hard problem to start with, multi-disjointness. It also introduces a new concept, called a \emph{promise problem}.
\begin{Exa}[Multi-Disjointness] $f(X_1,\ldots, X_n) = 1$ iff $X_i \cap X_j = \emptyset$ for all $i \neq j$. Moreover, we'll be interested in the \emph{promise problem}, where the protocol is promised that one of the following is true about the input:
\begin{itemize}
...
\begin{theorem}[\cite{NisanS06}]Every deterministic protocol for multi-disjointness has communication complexity $\Omega(k/n)$. 
\end{theorem}
\begin{proof}
...
\begin{theorem}[\cite{DobzinskiNS05}]The communication required to guarantee strictly better than a $(1/2+1/(2n))$-approximation for welfare maximization with $m$ items on all subadditive valuations is $2^{\Omega(m/n^2)}$.
\end{theorem}
...
Recall that we have a 2-approximation (that we didn't see) due to~\cite{Feige06}. This is asymptotically tight as $n\rightarrow \infty$, but not for small $n$ (actually, the algorithm is tight for small $n$, and the lower bound can be improved, see later section). 

\section{XOS Valuations}
\begin{theorem}[\cite{DobzinskiNS05}]The communication required to guarantee strictly better than a $1-(1-1/n)^n$-approximation for welfare maximization with $m$ items on all XOS valuations is $2^{\Omega(m/n^2)}$.
\end{theorem}
...
\begin{theorem}[\cite{BravermanMW18}] No simultaneous protocol with subexponential in $m$ communication can beat a $(3/4-1/108)$ approximation on the above construction with probability $> 2/3$ for decision.
...
\begin{definition}[$\ell$-sparse] We say that $\mathcal{S}$ is \emph{$\ell$-sparse} if for all $T_1,\ldots, T_{\ell-1} \in \mathcal{S}$, $\cup_j T_j \neq M$. \end{definition}
...
(Proof omitted). So now we have a class of subadditive functions. We now wish to come up with a hard instance. To this effect, we need one further definition.
